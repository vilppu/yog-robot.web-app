...
import { getHistory, authenticate, getOverview, getToken  } from "./agent-api.js";

var showDetails = (element, overviewEntry) => {
    getHistory(overviewEntry.sensorId, overviewEntry.measuredProperty).then(historyJson => {
...
var setupOverview = sensorsJson => {
    ...
    for (var sensor of sensors.sort(alphabeticOrder)) {
        ...
        var cloned = document.importNode(deviceTemplate.children[0], true);
...
export const setupSensorView = (setupSensorView) => {
    document.addEventListener('authenticated', e => {
        authenticate(e.detail.user.botId, e.detail.user.botKey)
            .then(() => {
                if (getToken() !== null) {
                    getOverview().then(setupOverview);
                }
            });
    }, false);
};
